...
def cross_validation(all_dataframe, fold_number=5):
    ...
    for train_index, valid_index in kfold.split(sequence_names, sequence_labels):
        ...
        model = EGMPPPIS(LAYER, INPUT_DIM, HIDDEN_DIM, NUM_CLASSES, DROPOUT)
        if torch.cuda.is_available():
            model.cuda()
...
if __name__ == "__main__":
    ...
    # 记录训练时长
    print(
        f"\n训练总时长: {int(total_duration // 3600)}小时 {int((total_duration % 3600) // 60)}分钟 {int(total_duration % 60)}秒")
